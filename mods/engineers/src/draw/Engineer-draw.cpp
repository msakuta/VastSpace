/** \file
 * \brief Implementation of Engineer class's drawing methods.
 */
#define NOMINMAX
...
Model *Engineer::model = NULL;
Motion *Engineer::motions[3] = {NULL};

void Engineer::draw(WarDraw *wd){
	Engineer *p = this;
	double pixels;
...
void Engineer::drawHookAndTether(WarDraw *wd){
...
void Engineer::drawOverlay(WarDraw *wd){
...
#if 1
void Engineer::drawtra(WarDraw *wd){
	Engineer *p = this;
...
void Engineer::onBulletHit(const Bullet *pb, int){
	if(w->getTeline3d()){
		Vec3d pos = pb->pos;
...
void Engineer::drawHUD(WarDraw *wd){
	char buf[128];
	Engineer *p = this;
	glPushMatrix();
...
bool Engineer::initModel(){
	static OpenGLState::weak_ptr<bool> init;

	if(!init){
		model = LoadMQOModel(modPath() << "models/Engineer.mqo");
		motions[0] = LoadMotion(modPath() << "models/Engineer_aim.mot");
		motions[1] = LoadMotion(modPath() << "models/Engineer_reload.mot");
		motions[2] = LoadMotion(modPath() << "models/Engineer_swapweapon.mot");
		init.create(*openGLState);
	}
...
bool Engineer::interpolate(MotionPose *mp){
	if(!initModel())
		return false;
...
bool Engineer::getGunPos(GetGunPosCommand &ggp){
	...
	if((ggp.gunId == 0 || ggp.gunId == 1) && interpolate(mp)){
		Vec3d srcpos;
		Quatd srcrot; 
		Engineer::model->getBonePos(ggp.gunId == 0 ? "rhand" : "body", *mp, &srcpos, &srcrot);
		Mat4d mat;
		transform(mat);
		srcpos *= modelScale;
		if(ggp.gunId == 0){
