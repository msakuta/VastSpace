...
struct MotionPose{
	...
	class NodeMap{
		typedef std::pair<dstring, MotionNode> Node; ///< Internal node type.
	...
	public:
		...
		iterator find(const dstring &a){
			for(int i = 0; i < n; i++) if(nodes[i]->first == a)
				return &nodes[i];
			return end();
		}
		const_iterator find(const dstring &a)const{
			return const_cast<const Node**>(const_cast<NodeMap*>(this)->find(a).p);
		}
		bool erase(const dstring &a){
			for(int i = 0; i < n; i++) if(nodes[i]->first == a){
				delete nodes[i];
				memmove(&nodes[i], &nodes[i+1], n - i - 1);
				return true;
			}
			return false;
		}
		MotionNode &operator[](const dstring &a){
			iterator it = find(a);
			if(it != end())
				return it->second;
			else
		...
		Node &push_back(const dstring &a){
...
inline bool Motion::load(std::istream &is){
	...
	for(int i = 0; i < nkfl; i++){
		...
		for(int j = 0; j < nnodes; j++){
			...
			newkfl.nodes[newname.c_str()] = node;
		}
