...
template<> void Entity::EntityRegister<Engineer>::sq_defineInt(HSQUIRRELVM v){
	...
	inventoryItemCtors["Thruster"] = [](double amount){return new ThrusterItem(int(amount));};
...
void Engineer::serialize(SerializeContext &sc){
	st::serialize(sc);
	sc.o << forcedRot;
	sc.o << forcedEnemy;
	sc.o << aiming;
...
void Engineer::unserialize(UnserializeContext &usc){
...
double Engineer::getHitRadius()const{
	return hitRadius;
}

double Engineer::getMaxHealth()const{
	return maxHealthValue;
}

bool Engineer::isTargettable()const{
	return true;
}

bool Engineer::isSelectable()const{
	return true;
}
...
#ifdef DEDICATED
void Engineer::draw(WarDraw *){}
void Engineer::drawtra(WarDraw *){}
void Engineer::drawHUD(WarDraw *){}
void Engineer::drawOverlay(WarDraw *){}
void Engineer::onBulletHit(const Bullet *, int){}
void Engineer::hookHitEffect(const otjEnumHitSphereParam &param){}
bool Engineer::getGunPos(GetGunPosCommand &ggp){
	ggp.pos = this->pos;
	ggp.rot = this->rot;
...
Engineer::Engineer(Game *g) : st(g){
	init();
}

Engineer::Engineer(WarField *w) : st(w){
	init();
}
...
void Engineer::init(){
	...
	if(game->isClient()){
		...
		if(!initialized){
			sq_init(modPath() << _SC("scripts/Engineer.nut"),
...
void Engineer::setPosition(const Vec3d *pos, const Quatd *rot, const Vec3d *velo, const Vec3d *avelo){
	if(bbody){
...
int Engineer::shoot_infgun(double phi0, double theta0, double v, double damage, double variance, double t, Mat4d &gunmat){
	Engineer *p = this;
	Bullet *pb;
...
//		return;
	...
	if(!controller && 0 < health){
		...
		if(arms[0] && arms[0]->ammo <= 0 && cooldown2 == 0. && !reloading){
			reload();
		}
...
//	QUATCNJIN(pt->rot);
	...
	/* shooter logic */
	p->muzzle = 0;
	if(arms[0] && !p->reloading && i & (PL_ENTER | PL_LCLICK) && !(i & PL_SHIFT)) while(p->cooldown2 < dt){
		amat4_t gunmat;
...
void Engineer::clientUpdate(double dt){
	anim(dt);
}
...
int Engineer::tracehit(const Vec3d &start, const Vec3d &dir, double rad, double dt, double *ret, Vec3d *retp, Vec3d *retnormal){
...
int Engineer::takedamage(double damage, int hitpart){
...
Entity::Props Engineer::props()const{
	Props ret = st::props();
...
int Engineer::armsCount()const{
	return numof(arms);
}

ArmBase *Engineer::armsGet(int i){
	return arms[i];
}

bool Engineer::command(EntityCommand *com){
...
SQInteger Engineer::sqGet(HSQUIRRELVM v, const SQChar *name)const{
...
SQInteger Engineer::sqSet(HSQUIRRELVM v, const SQChar *name){
	if(!scstrcmp(name, _SC("cooldown"))){
		SQFloat retf;
