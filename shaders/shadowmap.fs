...
float shadowMapIntensity(){
	if(	   0.001 < gl_TexCoord[4].x && gl_TexCoord[4].x < 0.999
		...
		return shadow2DProj(shadowmap3, gl_TexCoord[4])[3];
		...
		return shadow2DProj(shadowmap2, gl_TexCoord[3])[3];
	else{
		...
		return (shadow2DProj(shadowmap, gl_TexCoord[2]) * (1. - shadowedge) + shadowedge)[3];
	}
}
