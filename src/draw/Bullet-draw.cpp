...
void explotex(const Teline3CallbackData *pl, const Teline3DrawData *dd, void *pv){
	int i, j;
...
void explosmoke(const Teline3CallbackData *pl, const Teline3DrawData *dd, void *pv){
	int i, j;
...
#if 0
static void dirtsmoke(const Teline3CallbackData *pl, const Teline3DrawData *dd, void *pv){
	double rad;
	struct random_sequence rs;
	int i, n = (int)pv;
	...
	if(/*VECSDIST(pb->pos, wd->view) < .2 * .2*/ 1 < wpix){
		...
		if(true || damage < 500.){
			...
			if(!texname){
				...
				texname = CallCacheBitmap5("Bullet.bmp", "textures/Bullet.bmp", &stp, NULL, NULL);
			}
			glCallList(texname);
