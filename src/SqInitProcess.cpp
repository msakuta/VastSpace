...
/*	if(!game->isServer())
	...
	SQInteger len = sq_getsize(v, -1);
	if(-1 == len)
		throw SQFError(_SC("hardpoints size could not be acquired"));
	for(int i = 0; i < len; i++){
		sq_pushinteger(v, i); // root obj i
		if(SQ_FAILED(sq_get(v, -2))) // root obj obj[i]
			continue;
...
void HardPointProcess2::process(HSQUIRRELVM v)const{
	Game *game = (Game*)sq_getforeignptr(v);
	...
	for(int i = 0; i < len; i++){
		...
		if(SQ_FAILED(sq_get(v, -2))) // root obj obj[i]
			continue;
		std::unique_ptr<hardpoint_static> hs{std::make_unique<hardpoint_static>(game)};
		...
		hardpoints.push_back(std::move(hs));
		sq_poptop(v); // root obj
	}
	sq_poptop(v); // root
}
