...
static Game *server; ///< The server game.
...
//		warf.soundtime = TimeMeasLap(&tmwo) - dwo;
	...
	while(repeat){
		...
		// If we're client, read from TCP/IP stream.
...
struct HostGameDlgData{
	...
	FILE **plogfile;
	enum Application::GameMode gameMode;
	bool isClient()const{return gameMode == Application::ClientGame;}
};
...
static INT_PTR CALLBACK HostGameDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam){
	...
	case WM_INITDIALOG:
		SetWindowLongPtr(hDlg, DWLP_USER, (LONG_PTR)lParam);
		{
			...
			CheckDlgButton(hDlg, pd->gameMode == Application::ClientGame ? IDC_JOINGAME : pd->gameMode == Application::ServerGame ? IDC_HOSTGAME : IDC_STANDALONE, BST_CHECKED);
			SetDlgItemInt(hDlg, IDC_MAXCLIENTS, pd->params->maxclients, FALSE);
			EnableWindow(GetDlgItem(hDlg, IDC_MAXCLIENTS), pd->gameMode != Application::ServerGame ? FALSE : TRUE);
			SetDlgItemText(hDlg, IDC_NAME, pd->loginName);
			EnableWindow(GetDlgItem(hDlg, IDC_NAME), pd->gameMode == Application::ClientGame ? TRUE : FALSE);
	...
	case WM_COMMAND:
		{
			...
			if (id == IDOK || id == IDCANCEL) 
			{
				if(id == IDOK){
					...
					pd->gameMode = IsDlgButtonChecked(hDlg, IDC_JOINGAME) ? Application::ClientGame : IsDlgButtonChecked(hDlg, IDC_HOSTGAME) ? Application::ServerGame : Application::StandaloneGame;
			...
			else if(id == IDC_HOSTGAME || id == IDC_JOINGAME || id == IDC_STANDALONE){
				pd->gameMode = IsDlgButtonChecked(hDlg, IDC_JOINGAME) ? Application::ClientGame : IsDlgButtonChecked(hDlg, IDC_HOSTGAME) ? Application::ServerGame : Application::StandaloneGame;
...
int main(int argc, char *argv[])
{
	...
	do{
		...
		data.gameMode = application.mode;
		...
		if(IDCANCEL == dialogRet)
			return 0;
		application.mode = data.gameMode;

		// Check if joining the told server is feasible as early as possible.
		// If we fail here, we can retry it with other parameters.
		if(application.mode == Application::ClientGame && !application.joinGame(application.serverParams.hostname, application.serverParams.port))
			continue;
	...
	if(application.mode == application.ClientGame){
		server = NULL;
		application.clientGame = new ClientGame();
	}
	else if(application.mode == application.ServerGame){
...
/*		hWnd = CreateWindow(atom, "gltest", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
		...
		else if(application.mode == application.StandaloneGame)
			application.standalone(server);
		else
