...
int playWaveCustom(const char *lpszWAVEFileName, size_t delay, unsigned short vol, unsigned short pitch, signed char pan, unsigned short loops, char priority){
	...
	addsound((*node)->head.lpData, (*node)->head.dwBufferLength, delay, vol, pitch, pan, loops, priority);

	return 1;
}

int playWave3DCustom(const char *lpszWAVEFileName, const double pos[3], size_t delay, double vol, double attn, unsigned short pitch){
	static wavc_t *root = NULL;
	wavc_t **node;
	...
	return addsound3d((*node)->head.lpData, (*node)->head.dwBufferLength, delay, vol, attn, pos, pitch, (*node)->format.nSamplesPerSec);
}
...
int playWave3D(const char *fname, const double src[3], const double org[3], const double pyr[3], double vol, double attn, double delay){
	return playWave3DPitch(fname, src, org, pyr, vol, attn, delay, 256);
}
